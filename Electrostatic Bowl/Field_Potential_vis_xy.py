import numpy as np
import matplotlib.pyplot as plt
import Electric_Potential_and_Field_fn as myfns
import time

##Define the grid
x_grid = np.linspace(-2, 2, 200)
y_grid = np.linspace(-2, 2, 200)
X0, Y0 = np.meshgrid(x_grid, y_grid)

##Initialize the field components
Ex = np.zeros_like(X0)
EY = np.zeros_like(Y0)

##Initialize potential
phi = np.zeros_like(X0)

##total charge
Q = 1

print("The code is running ...")

##Measuring runtime while running
start_time = time.process_time()

##Potential calculation
for i in range(len(x_grid)):
    for j in range(len(y_grid)):
        x0 = X0[i, j]
        y0 = Y0[i, j]
        z0 = -0.5                                                                                             ##Calculations on xz-plane
        phi[i,j] = myfns.potential(x0, y0, z0, Q, 8.854e-12)

end_time = time.process_time()

time_taken = end_time - start_time
print(f"Time taken to run is: {time_taken:0.3f}")                                                          ##Printing Runtime


##Compute field
Ey, Ex = np.gradient(-phi)

##____________Potential Plot: Heatmap with colourbar_____________
plt.figure(figsize=(8, 6), dpi = 100)
contour = plt.contourf(X0, Y0, phi, levels=50, cmap='plasma')
plt.streamplot(X0, Y0, Ex, Ey, color='k', linewidth=0.7, density=2)

##Plot hemisphere arc
theta_arc = np.linspace(0, 2*np.pi, 200)
x_arc = np.sqrt(0.75)*np.cos(theta_arc)
y_arc = np.sqrt(0.75)*np.sin(theta_arc)
plt.plot(x_arc, y_arc, color='red', linewidth=3, label = "Shell")

plt.title(f'Potential & Electric Field Lines for total charge = {Q} C')
plt.xlabel('x axis (m)')
plt.ylabel('y axis (m)')
plt.colorbar(contour, label='Potential (V)')
plt.tight_layout()
plt.axis("equal")
plt.savefig("images/potential_field_no_point_charge_xy.png")
plt.show()
